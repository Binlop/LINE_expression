...
def main():
    ...
    coverage.add_argument('-c', '--cores', type=int, default=4, help='Количество ядер для использования')
    coverage.set_defaults(func=manager.coverage)

    coverage_cluster = subparsers.add_parser('cluster', help='Кластеризация покрытий экзонов относительно LINE')
    coverage_cluster.add_argument('-i', '--input', help='Название для итогового файла с покрытием', required=True)
    coverage_cluster.add_argument('-o', '--output', help='Название для итогового файла с кластеризованным покрытием', required=True)
    coverage_cluster.add_argument('-c', '--cores', type=int, default=4, help='Количество ядер для использования')
    coverage_cluster.set_defaults(func=manager.cluster)
    ...
    coverage_seed.add_argument("-c", "--cores", help="Кол-во ядер для расчета покрытия", required=False, default=1)
